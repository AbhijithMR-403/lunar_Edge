...
def check_wallet(request):
    if request.POST['isChecked'] == 'true':
        wallet_balance = float(user_profile.objects.get(account=request.user).wallet)
        wallet_null = True
        if wallet_balance == 0:
            wallet_null = False
            request.session['wallet'] = 0
        ...
        total_balance = total - wallet_balance
        if total_balance < 0:
            wallet_balance = abs(total_balance) + 1
            total_balance = 1
            # if total_balance
        wallet_used = total - total_balance
        print(wallet_used)

        request.session['wallet'] = total_balance
        if total > 2:
            wallet_balance = 0
        return JsonResponse({
            "check": wallet_null, "wallet_balance": wallet_balance,
            "total": total_balance, "wallet_used": wallet_used})
    else:
        return JsonResponse({"check": False})
